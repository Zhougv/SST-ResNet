...
from structure.encoder.gvp import AutoGraphEncoder

def iter_threading_map(func, data, workers: int = 64):
    pool = ThreadPool(workers)
    return pool.imap(func, data)

def threading_map(func, data, workers: int = 64):
    pool = ThreadPool(workers)
...
def generate_pos_subgraph(
    ...
    def quick_get_anchor_graph(anchor_node):
        ...
        if len(k_neighbors_indices) > 30:
            k_neighbors_indices = k_neighbors_indices[:10]
